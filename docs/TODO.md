...
|------------|------|--------|
| **CP1** (after Phase 2) | Config loads without errors | `uv run pytest tests/unit/test_config_loader.py tests/unit/test_config_models.py tests/unit/test_config_real_experiment.py tests/unit/test_config_real_hardware.py tests/unit/test_config_real_combined.py` |
| **CP2** (after Phase 3) | All providers pass protocol compliance | `uv run pytest tests/unit/test_providers.py tests/unit/test_provider_lifecycle.py tests/unit/test_llamacpp_load.py tests/unit/test_llamacpp_generate.py tests/unit/test_llamacpp_unload.py tests/unit/test_llamacpp_lifecycle.py` |
| **CP3** (after Phase 4) | Metrics + visualizer produce output from fake data | `uv run pytest tests/unit/test_metrics_collector.py tests/unit/test_metrics_sampler.py tests/unit/test_metrics_edge_cases.py tests/unit/test_visualizer_charts.py tests/unit/test_visualizer_table.py tests/unit/test_visualizer_generate_all.py` |
| **CP4** (after Phase 5) | All runners return valid metrics dict (mocked) | `uv run pytest tests/unit/test_cross_runner_metrics.py tests/unit/test_cross_runner_oom.py tests/unit/test_cross_runner_params.py` |
...
| **CP13** (after Phase 7) | Validation dry-run passes; no inference | `uv run python src/main.py --validate` |
...
| 10.5 | **cache_check.py undocumented in INTERFACES.md** | `shared/cache_check.py` is fully wired into `sdk_validation.py` (lines 15, 79) and used by `BenchmarkSDK.validate()` (the dry-run path). It is documented in `docs/TODO.md` (task 7.4) and `docs/PROMPT_LOG.md` but is missing from `docs/INTERFACES.md` — the "holy" contract document per CLAUDE.md §1. This violates the documentation-as-contract discipline. **Fix:** Add a new section (§11 or equivalent) in `docs/INTERFACES.md` documenting the `model_cache_status()` function and the `CacheCheckResult` dataclass. **Resolution:** Added `## 12. Cache Check — shared/cache_check.py` to `docs/INTERFACES.md`, documenting the `model_cache_status(model_ids: list[str]) -> dict[str, bool]` signature/contract and its consumption by `run_validation()` into `ValidationResult.models_cached` (§10). No `CacheCheckResult` dataclass exists in the current implementation — the function returns a plain `dict[str, bool]`, so the section documents the actual return type rather than inventing one. | ✅ Resolved |
| 10.6 | **CP1–CP12 checkpoint table outdated** | `docs/TODO.md`'s Integration Plan section references test filenames that no longer exist in the codebase (e.g., `test_config.py`, `test_providers.py` partially, `test_metrics.py`, `test_visualizer.py`, `test_runners.py`). The unit test suite was split into many more granular files (`test_config_loader.py`, `test_config_real_experiment.py`, `test_config_real_hardware.py`, `test_metrics_collector.py`, `test_metrics_sampler.py`, `test_visualizer_charts.py`, `test_gpu_runner.py`, etc.). Additionally, CP12 is defined twice (once for I13 / phase 7, once for I14 / phase 7.x — a numbering bug). **Fixes:** (a) Update all test-filename references in the checkpoint table to match current split-up test files. (b) Renumber the duplicate CP12 to CP13 or consolidate the checkpoints. | ✅ Resolved — CP1 now points at `test_config_loader.py`/`test_config_models.py`/`test_config_real_experiment.py`/`test_config_real_hardware.py`/`test_config_real_combined.py`; CP2 at `test_providers.py`/`test_provider_lifecycle.py`/the four `test_llamacpp_*.py` files; CP3 at `test_metrics_collector.py`/`test_metrics_sampler.py`/`test_metrics_edge_cases.py`/`test_visualizer_charts.py`/`test_visualizer_table.py`/`test_visualizer_generate_all.py`; CP4 at `test_cross_runner_metrics.py`/`test_cross_runner_oom.py`/`test_cross_runner_params.py`. The two checkpoints genuinely describe different gates (I13 "all three modes succeed" vs. Phase 7 "validation dry-run"), so the second `CP12` was renumbered to `CP13` rather than merged. |
| 10.7 | **Integration tier lacks GPU and AirLLM coverage** | `tests/integration/` contains exactly one real E2E test: `test_pipeline.py::test_full_pipeline_cpu_smoke` — CPU-only, validates `BenchmarkSDK.run_single()` end-to-end with a real 0.5B model and `TransformersProvider` on CPU. There is no integration-tier (as opposed to POC-tier) E2E test that exercises the GPU runner or the AirLLM runner end-to-end under real conditions. GPU and AirLLM coverage currently exists only in `tests/pocs/`, which is documented as proof-of-concept work, not the integration suite. **Fix:** Added `tests/integration/test_pipeline_gpu.py` with two real-hardware tests on this machine's RTX 3090: `test_full_pipeline_gpu_smoke` drives `BenchmarkSDK.run_single(mode="gpu_provider")` with `TransformersProvider(device="cuda")` on `Qwen/Qwen2.5-0.5B-Instruct`, asserting success status, positive load/TTFT/runtime, tokens generated, and `peak_vram_mb > 0` (measured: load_time_s≈0.18, ttft_s≈0.31, total_runtime_s≈12.5s, peak_vram_mb≈966); `test_full_pipeline_airllm_smoke` drives `run_single(mode="airllm")` with the same small model overridden in place of the 32B `large` tier (per PROMPT_LOG Entry 58, the full 32B AirLLM run takes ~18 min — disproportionate for a test), asserting success, positive load/runtime, tokens generated, and RAM usage (measured: load_time_s≈1.9, total_runtime_s≈18.7s, peak_vram_mb≈341); it also documents that `ttft_s` is expected to be exactly `0.0` for AirLLM since `AirllmRunner` never wires `MetricsCollector.mark_first_token` (only `TransformersProvider` does). Both tests are `skipif`-gated on `torch.cuda.is_available()`, same convention as the GPU/AirLLM PoC tests. Confirmed no regression: full suite passes with 93% coverage (>= 85% required); the pre-existing `ttft_s >= load_time_s` flake in `test_full_pipeline_cpu_smoke` and several PoC tests (triggered when the model is already HF-cached) is unrelated to this fix and left untouched. | ✅ Resolved |
| 10.8 | **LICENSE file decision still pending** | Noted in Phase 9 task 9.7 as deliberately left open: "license choice needs an explicit decision, not assumed." No `LICENSE` file is checked into the repo. **Action:** Decide on a license (MIT, Apache 2.0, etc.) and create the file, or confirm that the project is intentionally unlicensed and document that decision in a comment. **Resolution:** MIT was chosen. The `LICENSE` file already exists at the repo root (added in commit `8c21f37 chore: add MIT LICENSE`), and `README.md` correctly links to it (badge + License section). Verified valid MIT text; no further action needed. | ✅ Resolved |
| 10.9 | **ISO/IEC 25010 mapping never written** | Noted in Phase 9 task 9.7 as intentionally deferred: "checklist item, never required by this project's own PRD/PLAN/TODO." No ISO/IEC 25010 software-quality mapping doc exists. Per the project's own rules, this is not required — but it was a checklist item. **Action:** Confirm whether this is still intentionally out-of-scope or should be added as a reference doc. **Resolution:** Confirmed intentionally out-of-scope — `docs/PRD.md` and `docs/PLAN.md` were re-checked and neither references ISO/IEC 25010 as a requirement; no further doc will be added for it. | ✅ Resolved |
