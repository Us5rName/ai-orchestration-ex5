...
| 10.4 | **generate_visualization() has no CLI entry point** | `BenchmarkSDK.generate_visualization()` (sdk/sdk.py, lines 126–140) is fully implemented, tested, and documented (INTERFACES.md §1). It is reachable only programmatically (via the notebook) or by direct SDK instantiation — no `main.py` flag exposes it. Users cannot invoke visualization from the CLI. **Fix:** Add a `--visualize` (or equivalent) flag to `src/main.py` that delegates to `BenchmarkSDK.generate_visualization()`, enabling `uv run python src/main.py --visualize --config-dir <dir>`. | 🔄 Pending |
